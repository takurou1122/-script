...
public class PlayFabLogin : MonoBehaviour, IDetailedStoreListener
{
    ...
    private void ChangeText()
    {
        ...
        switch (setedPurchasedProductId)
        {
            ...
            case "hisyouhi_jyp.yokero":
                nonConsumableText.text = "購入されています。";
                _notad = 1;
                PlayerPrefs.SetInt("notad", _notad);//セーブデータに非消費型の課金がされたことを保存
                PlayerPrefs.Save();//セーブデータの保存
                Debug.Log(_notad);
                break;
